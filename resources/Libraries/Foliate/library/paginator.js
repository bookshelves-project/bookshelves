...
function animate(a, b, duration, ease, render) {
...
function makeRange(doc, node, start, end = start) {
...
function bisectNode(doc, node, cb, start = 0, end = node.nodeValue.length) {
  if (end - start === 1) {
...
function getBoundingClientRect(target) {
  let top = Infinity; let right = -Infinity; let left = Infinity; let bottom = -Infinity
...
function getVisibleRange(doc, start, end, mapRect) {
  ...
  const startOffset = from.nodeType === 1
    ? 0
...
function selectionIsBackward(sel) {
...
function setSelectionTo(target, collapse) {
  let range
...
function makeMarginals(length, part) {
...
function setStylesImportant(el, styles) {
  const { style } = el
...
export class Paginator extends HTMLElement {
  ...
  #onTouchMove(e) {
    ...
    e.preventDefault()
    const touch = e.changedTouches[0]
    const x = touch.screenX; const y = touch.screenY
    const dx = state.x - x; const dy = state.y - y
