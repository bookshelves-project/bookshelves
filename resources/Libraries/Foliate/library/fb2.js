...
async function parseXML(blob) {
  ...
  // so parse the XML declaration manually
    || str.match(/^<\?xml\s+version\s*=\s*["']1.\d+"\s+encoding\s*=\s*["']([\w.-]*)["']/)?.[1]
...
export async function makeFB2(blob) {
  const book = {}
  ...
  book.metadata = {
    ...
    contributor: $$('document-info author').map(getPerson).concat($$('document-info program-used').map(getElementText)).map(x => Object.assign(typeof x === 'string' ? { name: x } : x, { role: 'bkp' })),
