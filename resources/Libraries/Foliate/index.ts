...
export class FoliateEngine {
  ...
  public static async make(url: string | undefined): Promise<FoliateEngine> {
    ...
    const self = new FoliateEngine(file, file.size, response.headers)

    self.contentType = self.headers.get('content-type')
    self.contentDisposition = self.headers.get('content-disposition')

    const ebook = await self.handle()
    if (!ebook)
  ...
  public getContentType(): string | null | undefined {
    return this.contentType
  }

  public getContentDisposition(): string | null | undefined {
    return this.contentDisposition
  }

  public getType(): FoliateType {
    return this.type
  }
  ...
  public isFB2(): boolean {
    return this.isFb2
  }

  public isEPUB(): boolean {
    return this.isEpub
  }

  public isCBZ(): boolean {
    return this.isCbz
  }

  public isFBZ(): boolean {
    return this.isFbz
  }

  public getEbook(): Ebook | undefined {
    return this.ebook
  }
  ...
  private async handleZip() {
    ...
    if (this.isEpub) {
      ...
      const epub = await new EPUB(loader).init() as unknown as Ebook
      this.ebook = epub
