...
function concatTypedArray3(a, b, c) {
...
function getStruct(def, buffer) {
  return Object.fromEntries(Array.from(Object.entries(def))
    .map(([key, [start, len, type]]) => [key, (type === 'string' ? getString : getUint)(buffer.slice(start, start + len))]))
}
...
async function huffcdic(mobi, loadRecord) {
...
async function getIndexData(indxIndex, loadRecord) {
...
async function getNCX(indxIndex, loadRecord) {
...
async function getFont(buf, unzlib) {
...
export async function isMOBI(file) {
...
export class MOBI extends PDB {
  ...
  async getCover() {
    ...
    const offset = exth?.coverOffset < 0xFFFFFFFF
      ? exth?.coverOffset
...
async function replaceSeries(str, regex, f) {
  const matches = []
...
function getPageSpread(properties) {
...
class KF8 {
  ...
  async init() {
    ...
    this.#sections = skelTable.reduce((arr, skel) => {
      const last = arr[arr.length - 1]
      const fragStart = last?.fragEnd ?? 0; const fragEnd = fragStart + skel.numFrag
  ...
  async loadRaw(start, end) {
    ...
    const distanceEnd = this.#fullRawLength == null
      ? Infinity
