...
function concatArrays(a, b) {
  return a.slice(0, -1).concat([a[a.length - 1].concat(b[0])]).concat(b.slice(1))
}
...
function tokenizer(str) {
  const tokens = []
  let state; let escape; let value = ''
...
function parserIndir(tokens) {
  return splitAt(tokens, findTokens(tokens, '!')).map(parser)
}
...
function buildRange(from, to) {
  ...
  const localFrom = from[from.length - 1]; const localTo = to[to.length - 1]
  const localParent = []; const localStart = []; const localEnd = []
  let pushToParent = true
  const len = Math.max(localFrom.length, localTo.length)
  for (let i = 0; i < len; i++) {
    const a = localFrom[i]; const b = localTo[i]
...
function getChildNodes(node, filter) {
...
function indexChildNodes(node, filter) {
...
function partsToNode(node, parts, filter) {
...
export function fromRange(range, filter) {
...
export function toRange(doc, parts, filter) {
...
export function toElement(doc, parts) {
  return partsToNode(doc.documentElement, collapse(parts)).node
}
...
export function fromCalibrePos(pos) {
  const [parts] = parse(pos)
  const item = parts.shift()
  parts.shift()
  return toString([[{ index: 6 }, item], parts])
}
export function fromCalibreHighlight({ spine_index, start_cfi, end_cfi }) {
