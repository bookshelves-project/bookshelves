...
function filterAttribute(attr, value, isList) {
  return isList
...
function getMetadata(opf) {
  ...
  for (const [k, el] of getMetasByPrefix('media:')) {
    ...
    else if (k === 'active-class') {
      media.activeClass = v
    }
    else if (k === 'narrator') {
      media.narrator.push(v)
    }
    else if (k === 'playback-active-class') {
      media.playbackActiveClass = v
    }
  }
...
function parseNav(doc, resolve = f => f) {
  ...
  const $$nav = $$$(doc, 'nav')
  let toc = null; let pageList = null; let landmarks = null; const others = []
...
async function deobfuscate(key, length, blob) {
...
async function WebCryptoSHA1(str) {
  const data = new TextEncoder().encode(str)
  const buffer = await globalThis.crypto.subtle.digest('SHA-1', data)
  return new Uint8Array(buffer)
}

function deobfuscators(sha1 = WebCryptoSHA1) {
  return {
...
class Loader {
  ...
  async replaceCSS(str, href, parents = []) {
    ...
    // replace vw and vh as they cause problems with layout
      .replace(/(\d+(?:\.\d+)?|\.\d+)vw/gi, (_, d) => `${Number.parseFloat(d) * w / 100}px`)
...
function getHTMLFragment(doc, id) {
...
function getDisplayOptions(doc) {
  if (!doc)
    return null
  return {
