...
export function isOPDSCatalog(str) {
...
function useNS(doc, ns) {
  return doc.lookupNamespaceURI(null) === ns || doc.lookupPrefix(ns) ? ns : null
}
...
function getIndirectAcquisition(el) {
...
export function getPublication(entry) {
  ...
  const filterDC = (x) => {
    const a = filterDCEL(x); const b = filterDCTERMS(x)
    return y => a(y) || b(y)
  }
...
export function getOpenSearch(doc) {
