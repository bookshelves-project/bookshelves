...
function getLang(el) {
  ...
  return x || (el.parentElement ? getLang(el.parentElement) : null)
}

function getAlphabet(el) {
  const x = el?.getAttributeNS?.(NS.XML, 'lang')
  return x || (el.parentElement ? getAlphabet(el.parentElement) : null)
}

function getSegmenter(lang = 'en', granularity = 'word') {
...
function fragmentToSSML(fragment, inherited) {
...
function getFragmentWithMarks(range, textWalker, granularity) {
