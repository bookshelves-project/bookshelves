...
async function makeDirectoryLoader(entry) {
  const entries = await getFileEntries(entry)
  const files = await Promise.all(
    entries.map(entry => new Promise((resolve, reject) =>
      entry.file(file => resolve([file, entry.fullPath]), error => reject(error)))),
  )
...
function isCBZ({ name, type }) {
  return type === 'application/vnd.comicbook+zip' || name.endsWith('.cbz')
}

function isFB2({ name, type }) {
  return type === 'application/x-fictionbook+xml' || name.endsWith('.fb2')
}

function isFBZ({ name, type }) {
  return type === 'application/x-zip-compressed-fb2'
    || name.endsWith('.fb2.zip') || name.endsWith('.fbz')
}

async function getView(file) {
  let book
  if (file.isDirectory) {
...
function getCSS({ spacing, justify, hyphenate }) {
  return `
...
class Reader {
  ...
  async open(file) {
    ...
    $('#side-bar-author').innerText = typeof author === 'string'
      ? author
      : author
