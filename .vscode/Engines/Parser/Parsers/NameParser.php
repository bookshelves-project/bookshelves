...
class NameParser
{
    ...
    {
        $filename = pathinfo($parser_engine->fileName(), PATHINFO_FILENAME);
    ...
    private static function extractCreators(?string $creators): array
    {
        ...
        if (null !== $creators) {
            ...
            foreach ($creators as $creator) {
                array_push($list, new BookEntityAuthor(NameParser::transformToString($creator), 'aut'));
            }
        }
    ...
    private static function extractIdentifiers(?string $identifiers = null): array
    {
        if ($identifiers) {
            ...
            foreach ($identifiers as $identifier) {
                array_push($list, new BookEntityIdentifier('isbn13', NameParser::transformToString($identifier)));
            }
