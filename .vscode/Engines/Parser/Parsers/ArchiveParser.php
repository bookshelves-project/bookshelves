...
class ArchiveParser
{
    ...
    public function open(): ParserEngine|false
    {
        ...
        $this->engine->pageCount = count($this->zip_files_list);
    ...
    public function rar(): static|false
    {
        ...
        foreach ($zip->getEntries() as $key => $entry) {
            ...
            if ($ext === $this->extension_index) {
                $xml_path = "{$path}/{$this->engine->fileName()}.{$ext}";
        ...
        if ($this->engine->coverName()) {
            foreach ($zip->getEntries() as $key => $entry) {
                if ($this->engine->coverName === $entry->getName()) {
                    ...
                    $cover_path = "{$path}/{$this->engine->fileName()}.{$extension}";
    ...
    public function zip(): static|false
    {
        ...
        if ($this->engine->coverName()) {
            for ($i = 0; $i < $zip->numFiles; $i++) {
                $cover = $zip->getFromName($this->engine->coverName());
                $this->engine->coverFile = base64_encode($cover);
            }
        }
    ...
    private function sortFileList(array $zip_files_list)
    {
        if ($this->find_cover) {
            ...
            if (count($this->zip_files_list) > 0) {
                $this->engine->coverName = $this->zip_files_list[0];
            }
        }
    }
